...
class FootSpec:
    """Geometry of a plain glue-on foot.

    The foot is a smooth, flat-topped pad: glue its top face anywhere under a
    tray. There is no peg and no matching socket -- the body stays a clean flat
    bottom, and you decide where to place the feet. Modeled flat on z=0 so it
    prints without supports.
    """

    foot_radius: float = 14.0       # ground-contact / glue radius (load spreading)
    foot_height: float = 16.0       # base height = ground clearance under the body
    base_fillet: float = 4.0        # rounded bottom (ground) edge
    top_fillet: float = 2.0         # small rounded top (glue-face) edge


def plain_foot(spec: FootSpec) -> cq.Workplane:
    """A smooth cylindrical foot to glue under a tray (no peg, no socket)."""
    ...
    top_fillet = min(spec.top_fillet, spec.foot_radius * 0.3, spec.foot_height * 0.3)
    if top_fillet > 0:
        foot = foot.edges(">Z").fillet(top_fillet)
    return foot
